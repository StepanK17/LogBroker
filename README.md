...
Иными словами, это проект не просто про брокер сообщений, а именно про попытку воспроизвести основу подхода Kafka:
...
Если запускаешь сценарий повторно, используй новое имя `topic` или начни с пустого каталога `data/`, потому что повторное создание уже существующего `topic` вернет ошибку.
